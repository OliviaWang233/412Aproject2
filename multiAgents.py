...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        ghostDis = util.manhattanDistance(newPos, ghostPos)
        foodsPos = newFood.asList()
        nearsetDis = 100000
    
        for foodPos in foodsPos:
            if nearsetDis > util.manhattanDistance(newPos, foodPos):
                nearsetDis = util.manhattanDistance(newPos, foodPos)
        score = successorGameState.getScore()
        if ghostDis < 3:
            score = score + ghostDis*10
        score = score + 10/nearsetDis
        if len(foodsPos) < len(currentGameState.getFood().asList()):
            score = score +100
        return score 
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        action = self.minimax(gameState, 0, self.depth)[1]
        return action
    ...
    def min(self, gameState, agentIndex, depth):
        actions = []
        for action in gameState.getLegalActions(agentIndex):
            a = (self.minimax(gameState.generateSuccessor(agentIndex, action), agentIndex + 1, depth)[0], action)
            actions.append(a)    
        return min(actions)
